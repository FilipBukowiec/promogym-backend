...
export class MediaService {
  ...
  async upload(createMediaDto: CreateMediaDto, tenant_id: string): Promise<Media> {
    await this.mediaModel.updateMany(
      { tenant_id, order: { $gte: 1 } },
      { $inc: { order: 1 } }
    );

    const newMedia = new this.mediaModel({ ...createMediaDto, tenant_id, order: 1 });
