...
export class NewsService {
  ...
  async deleteNews(id: string, tenant_id: string): Promise<void> {
    const deletedNews = await this.newsModel.findOneAndDelete({
      _id: id,
      tenant_id,
    });
    if (!deletedNews) {
  ...
  async moveUp(id: string, tenant_id: string): Promise<void> {
    ...
    try {
      const currentNews = await this.newsModel
        .findOne({ _id: id, tenant_id })
        .session(session);
      if (!currentNews) throw new NotFoundException('News not found');

      const aboveNews = await this.newsModel
        .findOne({
          tenant_id,
      ...
      await this.newsModel
        .updateOne({ _id: currentNews._id }, { order: currentNews.order - 1 })
        .session(session);
      await this.newsModel
        .updateOne({ _id: aboveNews._id }, { order: aboveNews.order + 1 })
        .session(session);
  ...
  async moveDown(id: string, tenant_id: string): Promise<void> {
    ...
    try {
      ...
      const belowNews = await this.newsModel
        .findOne({
          tenant_id,
          order: currentNews.order + 1,
        })
      ...
      await this.newsModel
        ...
        .session(session);
      await this.newsModel
        .updateOne({ _id: belowNews._id }, { order: belowNews.order - 1 })
        .session(session);
